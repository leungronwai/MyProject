...
# Use this line to import the gain matrix Gx and matrix P from the optimization procress:使用这一行从优化过程中导入增益矩阵Gx和矩阵P
...
Fs = 100
dt = 1/Fs
nd = 6       # Number of differential equations微分方程个数
N = 300000   # Number of points 点数
t0 = 0
tf = N*dt
...
#%% Vector for the membership functions: 成员函数向量
F1pp = []
F2pp = []
F3pp = []
F4pp = []
...
#%% Set of differential equations in matrix form: 组矩阵形式的微分方程集
def __dXdt__( Xd, g ):
    ...
    # States to which the controller drives the system after it is on: 控制器打开系统后驱动系统的状态
